...
# Upload zip file
# the zip file is called 'lys_structures' and contains 206 lysozyme structures with 100% sequence similarity
...
# List all PDB files
pdb_files = []
for root, dirs, files in os.walk('lys_structures'):
    for file in files:
...
# Login to Hugging Face Hub and Input hugging face token
# Then load ems small model and encoder 
login(token='', add_to_git_credential=True)

model: ESM3InferenceClient = ESM3.from_pretrained("esm3_sm_open_v1").to("cuda")
encoder = ESM3_structure_encoder_v0("cuda")

# process structures and generate structure tokens
...
# calculate token Similarity and RMSD to reference pdb structure
...
# Use '6lyt_A.pdb' as the reference file
reference_file = '6lyt_A.pdb'
reference_path = os.path.join('lys_structures', reference_file)
...
if reference_tokens is not None:
    ...
    for pdb_file, tokens in all_structure_tokens.items():
        if os.path.basename(pdb_file) != reference_file:
            ...
            try:
                rmsds[os.path.basename(pdb_file)] = calculate_rmsd(reference_path, os.path.join('lys_structures', os.path.basename(pdb_file)))
            except Exception as e:
    ...
    plt.title('ESM3 Token Similarity vs RMSD for Lysozyme Structures')
    plt.grid(False)
